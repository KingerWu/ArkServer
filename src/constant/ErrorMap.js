...
class Err {
    ...
    to(res, extra) {
        if (extra) {
            res.status(this.httpCode).send(JSON.stringify({
                "code": this.errCode,
...
const ErrMap = {
    ...
    EmailCheckFail: new Err(400, 10011, "邮箱格式错误"),
    EmailLockFail: new Err(403, 10012, "邮箱已锁定，请稍后尝试"),
    EmailSended: new Err(403, 10013, "邮件已发送，请勿重复点击"),
    EmailTypeError: new Err(400, 10014, "验证码类型有误"),
    EmailRegisteredError: new Err(403, 10015, "邮箱已注册"),
    CodeError: new Err(403, 10016, "验证码过期或错误"),
    EmailNotRegisteredError: new Err(403, 10017, "邮箱未注册"),

    ApiTooLow: new Err(403, 13000, "请升级到最新的客户端"),
    SignCheckFail: new Err(403, 13001, "接口验证签名失败"),
    SignParamLost: new Err(403, 13002, "接口签名字段缺失, 缺失字段"),
    RequestParamLost: new Err(403, 13003, "请求字段缺失, 缺失字段"),
